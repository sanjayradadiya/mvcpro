...
{
    public class ClientEditProfileVM
    {
        ...
        [Remote("checkUser", "Account", HttpMethod = "POST", ErrorMessage = "User name already exists. Please enater a different one..")]
        [Display(Name = "Email")]
        ...
        //[Required(ErrorMessage = "Confirm Password could not be blank..")]
        ...
        [NotMapped]
        public IEnumerable<SelectListItem> ListSalesMgr { get; set; }

        [NotMapped]
        public IEnumerable<SelectListItem> ListHandlingLocation { get; set; }

        [NotMapped]
        public IEnumerable<SelectListItem> ListEmailType { get; set; }

        [NotMapped]
        public IEnumerable<SelectListItem> ListUserLevels { get; set; }
        ...
        public static IEnumerable<SelectListItem> GetClientStatus()
        {
            return (from ClientStatus s in Enum.GetValues(typeof(ClientStatus))
                    select new { ID = s, Name = s.ToString() }).Select(x => new SelectListItem { Text = x.ID.ToString() + ".", Value = x.Name });
        }
        ...
        public static IEnumerable<SelectListItem> GetHandlingLocation()
        {

            string vQry = "SELECT ORD,DBO.INITCAP(PROCNM) NM FROM PROCMAS WHERE PROCGROUP = 'LOCATION' ORDER BY ORD";
            DataTable dt = ComFunc.excDataTable(vQry);
            return dt.AsEnumerable().Select(x => new { CountryCd = x.Field<decimal>("ORD"), CountryNm = x.Field<string>("NM") })
                .Select(x => new SelectListItem { Text = x.CountryNm, Value = Convert.ToString(x.CountryCd) });
        }

        public static IEnumerable<SelectListItem> GetSalesManager()
        {

            string vQry = "SELECT CLIENTCD,DBO.INITCAP(TITLE) + ' ' + DBO.INITCAP(FIRSTNAME) + ' ' + DBO.INITCAP(LASTNAME) USERNAME FROM CLIENTMASTER WHERE UTYPEID=3";
            DataTable dt = ComFunc.excDataTable(vQry);
            return dt.AsEnumerable().Select(x => new { clientCd = x.Field<Int32>("CLIENTCD"), ClientNm = x.Field<string>("USERNAME") })
                .Select(x => new SelectListItem { Text = x.ClientNm, Value = Convert.ToString(x.clientCd) });
        }

        public static IEnumerable<SelectListItem> GetEmailSchedule()
        {

            string vQry = "SELECT ORD,DBO.INITCAP(PROCNM) NM FROM PROCMAS WHERE PROCGROUP = 'EMAILSTATUS' ORDER BY ORD";
            DataTable dt = ComFunc.excDataTable(vQry);
            return dt.AsEnumerable().Select(x => new { ord = x.Field<decimal>("ORD"), procNm = x.Field<string>("NM") })
                .Select(x => new SelectListItem { Text = x.procNm, Value = Convert.ToString(x.ord) });
        }

        public static IEnumerable<SelectListItem> GetUserLevels()
        {

            string vQry = "SELECT PROCCD ORD, SHORTNM NM FROM PROCMAS WHERE PROCGROUP='USERTYPE'";
            DataTable dt = ComFunc.excDataTable(vQry);
            return dt.AsEnumerable().Select(x => new { ord = x.Field<decimal>("ORD"), procNm = x.Field<string>("NM") })
                .Select(x => new SelectListItem { Text = x.procNm, Value = Convert.ToString(x.ord) });
        }
